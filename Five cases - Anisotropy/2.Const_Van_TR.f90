	...
	!***********************************************************************
	    ...
	    do j =1, NY
	    	...
	    	do i=1,NX
		        ...
		        error_inf	=	(MAX(error_inf,ABS(cons(1,i,j)-cons_exact(1,i,j))))/MAXVAL(cons_exact(1,:,:))
		        ! error_inf	=	(MAXVAL(ABS(cons(1,1:NX,1:NY)-cons_exact(1,1:NX,1:NY))))/MAXVAL(cons_exact(1,:,:))
...
! ! 	! Top boundary conditions
 ...
 !    enddo
	...
	!***************************************
		 ...
		 jacobian(1,3) = 0.0d0
		 jacobian(2,1) = 0.0d0
		 jacobian(2,2) = Tr*abs(sqrt(d_xx/Tr))*d_xx/d_xx
		 jacobian(2,3) = Tr*abs(sqrt(d_xx/Tr))*d_xy/d_xx
		 jacobian(3,1) = 0.0d0
		 jacobian(3,2) = 0.0d0
		 jacobian(3,3) = 0.0d0
	...
	!***************************************
		 ...
		 jacobian(2,3) = 0.0d0
		 jacobian(3,1) = 0.0d0
		 jacobian(3,2) = Tr*abs(sqrt(d_yy/Tr))*d_yx/d_yy
		 jacobian(3,3) = Tr*abs(sqrt(d_yy/Tr))*d_yy/d_yy
	...
	!***********************************************************************
 	...
 	subroutine sourceterm(NX,NY,x,y,cons,flux_x,flux_y,residual,n_eqn,ghostp)
		...
		double precision					:: A, B, C, D, E, F,frac
			...
			do j = 1, NY
				...
				frac = -((sin(pi*x(i))*sin(pi*y(j)))**gamma)/(sin(pi*x(i))*sin(pi*x(i))*sin(pi*y(j))*sin(pi*y(j))*tan(pi*x(i))*tan(pi*y(j)))

				A = (sin(pi*x(i))*sin(pi*x(i))*sin(pi*y(j))*sin(pi*y(j)))*sin(beta*pi/180.0d0)*cos(beta*pi/180.0d0)*2.0d0*pi*pi*gamma*gamma*x(i)*y(j)*(D_parallel-1.0d0)

				B = (tan(pi*x(i))*tan(pi*y(j)))*sin(pi*x(i))*sin(pi*x(i))*pi*pi*gamma*x(i)*y(j)*(D_parallel*gamma*sin(beta*pi/180.0d0)*sin(beta*pi/180.0d0)-D_parallel*sin(beta*pi/180.0d0)*sin(beta*pi/180.0d0) &
					 -gamma*sin(beta*pi/180.0d0)*sin(beta*pi/180.0d0)+sin(beta*pi/180.0d0)*sin(beta*pi/180.0d0)+gamma - 1.0d0)

				C = (tan(pi*x(i))*tan(pi*y(j))*sin(pi*y(j))*sin(pi*y(j)))*pi*pi*gamma*x(i)*y(j)*(D_parallel*gamma*cos(beta*pi/180.0d0)*cos(beta*pi/180.0d0)-D_parallel*cos(beta*pi/180.0d0)*cos(beta*pi/180.0d0)&
					 -gamma*cos(beta*pi/180.0d0)*cos(beta*pi/180.0d0)+cos(beta*pi/180.0d0)*cos(beta*pi/180.0d0)+gamma - 1.0d0)

				D = sin(pi*x(i))*sin(pi*x(i))*sin(pi*y(j))*sin(pi*y(j))*tan(pi*x(i))*2.0d0*pi*gamma*(D_parallel*x(i)*sin(beta*pi/180.0d0)*sin(beta*pi/180.0d0)+0.5d0*D_parallel*y(j)*sin(2.0d0*beta*pi/180.0d0)&
					  -x(i)*sin(beta*pi/180.0d0)*sin(beta*pi/180.0d0)+x(i)-0.5d0*y(j)*sin(2.0d0*beta*pi/180.0d0))

				E = sin(pi*x(i))*sin(pi*x(i))*sin(pi*y(j))*sin(pi*y(j))*tan(pi*y(j))*2.0d0*pi*gamma*(0.5d0*D_parallel*x(i)*sin(2.0d0*beta*pi/180.0d0)-D_parallel*y(j)*sin(beta*pi/180.0d0)*sin(beta*pi/180.0d0)&
					   +D_parallel*y(j)-0.5d0*x(i)*sin(2.0d0*beta*pi/180.0d0)+y(j)*sin(beta*pi/180.0d0)*sin(beta*pi/180.0d0))

				F = -sin(pi*x(i))*sin(pi*x(i))*sin(pi*y(j))*sin(pi*y(j))*tan(pi*x(i))*tan(pi*y(j))*(pi*pi*D_parallel*gamma*gamma*x(i)*y(j)-D_parallel*sin(2.0d0*beta*pi/180.0d0)+pi*pi*gamma* &
					gamma*x(i)*y(j)+sin(2.0d0*beta*pi/180.0d0))

				sourcet(1,i,j)		= (A+B+C+D+E+F)*frac
	...
	open(unit=1,file='solution_2.plt')
