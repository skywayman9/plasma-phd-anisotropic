...
!**********************************************************************
! This program solves 2D Diffusion equation with verying Tensor
! Nishikawa AIAA 2017-0310
!**********************************************************************
	...
	program hyperpoi2d
		...
		integer, parameter					:: NTMAX=4000000,l=2
		integer 							:: i, j, z, k, NT
		
		integer, parameter					:: NX = 8*l,NY = 8*l, n_eqn = 3, ghostp = 10 	
...
dt =1.0d10
	...
	write(*,*)
  	...
  	write(*,*)
		...
		write(*,'(a15,4es12.4,4es12.4,4es12.4)') "  Residual norm (L1) = ", res_norm_initial(:,L1)
		write(*,'(a15,4es12.4,4es12.4,4es12.4)') "  Residual norm (L2) = ", res_norm_initial(:,L2)
		write(*,'(a15,4es12.4,4es12.4,4es12.4)') "  Residual norm (Li) = ", res_norm_initial(:,Linf)
		write(*,*)
	...
	double precision 					:: dx, dy, xmin, xmax, ymin, ymax
		...
		do j = 1, NY
			...
			do i = 1, NX
				...
				cons_exact(1,i,j) = (1.0d0-( ( (x(i)-0.5d0)**2.0d0+(y(j)-0.5d0)**2.0d0)**1.50d0))
	...
	!***********************************************************************
 	...
 	double precision, parameter 		:: pi=acos(-1.0d0), D_parallel = 1.0d9
 	integer 							:: i, j
	...
	dt = 1.0d10 
		...
		do i = 1, NX
			...
			d_xx = D_parallel*(y(j)**2 + (x(i) + 1.0d0)**2)
			d_xy = -x(i)*y(j)
 ...
 ! ! ! Left boundary conditions (actually left)
	...
	do j = 1,NY
	    ...
	    cons(1,i,j )   = (8.0d0*(1.0d0-(((((x(i)+x(i+1))*0.5d0)-0.5d0)**2.0d0+(y(j)-0.5d0)**2.0d0)**1.50d0)) - 6.0d0*cons(1,i+1,j) + cons(1,i+2,j)) * (1.0d0/3.0d0)
	...
	do j = 1, NY
		...
		cons(1,i+1,j) = (8.0d0*(1.0d0-(((((x(i)+x(i+1))*0.5d0)-0.5d0)**2.0d0+(y(j)-0.5d0)**2.0d0)**1.50d0)) +1.0d0*cons(1,i-1,j) -6.0d0* cons(1,i,j)) * (1.0d0/3.0d0)
	...
	do i = 1, NX
		...
		cons(1,i,j+1) = (8.0d0*(1.0d0-(((((y(j)+y(j+1))*0.5d0)-0.5d0)**2.0d0+(x(i)-0.5d0)**2.0d0)**1.50d0)) +1.0d0*cons(1,i,j-1) -6.0d0* cons(1,i,j)) * (1.0d0/3.0d0)
	...
	do i = 1,NX
		j=0     
	    cons(1,i,j )   = (8.0d0*(1.0d0-(((((y(j)+y(j+1))*0.5d0)-0.5d0)**2.0d0+(x(i)-0.5d0)**2.0d0)**1.50d0)) - 6.0d0*cons(1,i,j+1) + cons(1,i,j+2)) * (1.0d0/3.0d0)
	...
	do j =1, NY
		...
		do i = -ghostp, NX+ghostp
			...
			d_xx(i) = D_parallel*(y(j)**2 + (x(i) + 1.0d0)**2)
			d_xy(i) = -x(i)*y(j)
...
!***************************************************************************************************************
	...
	!***************************************
		 ...
		 jacobian(2,3) = Tr*abs(sqrt(((d_xxl(i)+d_xxr(i))*0.5d0)/Tr))*(d_xyl(i)+d_xyr(i))*0.5d0/((d_xxl(i)+d_xxr(i))*0.5d0)
		 jacobian(3,1) = 0.0d0
		 jacobian(3,2) = 0.0d0
		 jacobian(3,3) = 0.0d0
	...
	do j =1, NX
		...
		do i = -ghostp, NY+ghostp
			...
			d_xx(i) = D_parallel*(y(i)**2 + (x(j) + 1.0d0)**2)
			d_xy(i) = -x(j)*y(i)
			...
			Lr = 2.0d0/(acos(-1.0d0)*(acos(-1.0d0)*(1.0d0/NX)+4.0d0))
			Tr = (2.0d0*Lr**2.0d0)/(d_xx(i)+d_yy(i)+2.0d0*d_xy(i))
...
!***************************************************************************************************************
	...
	!***************************************
		 ...
		 !***** Roe solver          *****
		 !The upwind dissipation matrix is |A| = R*|Lambda|*L
		 jacobian(1,1) = abs(sqrt(((d_yyl(i)+d_yyr(i))*0.5d0)/Tr))
		 jacobian(1,2) = 0.0d0
		 jacobian(1,3) = 0.0d0
		 jacobian(2,1) = 0.0d0
		 jacobian(2,2) = 0.0d0
		 jacobian(2,3) = 0.0d0
		 jacobian(3,1) = 0.0d0
		 jacobian(3,2) = Tr*abs(sqrt(((d_yyl(i)+d_yyr(i))*0.5d0)/Tr))*(d_xyl(i)+d_xyr(i))*0.5d0/((d_yyl(i)+d_yyr(i))*0.5d0)
		 jacobian(3,3) = Tr*abs(sqrt(((d_yyl(i)+d_yyr(i))*0.5d0)/Tr))
...
my_function = ((x - 0.5d0)**2 + (y - 0.5d0)**2)**(-0.5d0)*(D_para*(1.0d0 &
      *x - 0.5d0)*(3.0d0*x - 1.5d0)*(y**2 + (x + 1)**2) - x*y*(1.0d0*x &
      - 0.5d0)*(3.0d0*y - 1.5d0) - x*y*(3.0d0*x - 1.5d0)*(1.0d0*y - &
      0.5d0) + (1.0d0*y - 0.5d0)*(y + 1)**2*(3.0d0*y - 1.5d0) + ((x - &
      0.5d0)**2 + (y - 0.5d0)**2)**1.0d0*(2*D_para*(x + 1)*(3.0d0*x - &
      1.5d0) + 3.0d0*D_para*(y**2 + (x + 1)**2) - x*(3.0d0*x - 1.5d0) - &
      y*(3.0d0*y - 1.5d0) + 3.0d0*(y + 1)**2 + 2*(y + 1)*(3.0d0*y - &
      1.5d0)))
	...
	open(unit=1,file='solution_3b.plt',form='formatted',status='replace')
