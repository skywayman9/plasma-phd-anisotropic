...
!	 Written by Sainath Ch, Email: s.chamarthi@al.t.u-tokyo.ac.jp
	...
	program hyperpoi2d
		...
		integer, parameter					:: NTMAX=100000,l=16
		integer 							:: i, j, z, k, NT
		
		integer, parameter					:: NX = 16*l,NY = 16*l, n_eqn = 3, ghostp = 15	
...
! pause
	...
	!***************************************************************************************************
		...
		write(*,*)
		write(*,*) "Finished solver... See you later alligator! "
		endif
		write(*,*) 'Program ends...'
	...
	dt = 1.0d10;
		...
		do i = 1, NX
			...
			do j = 1, NY
					...
					d_xx = D_parallel*(1.0d0+cons(1,i,j)**2.0d0)
					d_xy = 0.0d0
					d_yx = 0.0d0
					d_yy = 1.0d0+cons(1,i,j)**2.0d0
 ...
 ! ! Left boundary conditions
	...
	do j =1, NY
		...
		do i = -ghostp, NX+ghostp
			...
			cons_local(3,i) = cons(3,i,j)
					...
					d_xx(i) = D_parallel*(1.0d0+cons(1,i,j)**2.0d0)
					d_xy(i) = 0.0d0
					d_yx(i) = 0.0d0
					d_yy(i) = 1.0d0+cons(1,i,j)**2.0d0
	...
	double precision 					:: d_yyl(-ghostp:NY+ghostp),d_yyr(-ghostp:NY+ghostp)
		...
		do k=1,n_eqn
        	...
        	do i =-2,NX+2
              ...
              consl(k,i) = -1.0d0/6.0d0*cons_local(k,i-1)+5.0d0/6.0d0*cons_local(k,i  )+1.0d0/3.0d0*cons_local(k,i+1)
              consr(k,i) =  1.0d0/3.0d0*cons_local(k,i  )+5.0d0/6.0d0*cons_local(k,i+1)-1.0d0/6.0d0*cons_local(k,i+2)
              ...
              fleft(k,i) =  -1.0d0/6.0d0*flux_node(k,i-1)+5.0d0/6.0d0*flux_node(k,i  )+1.0d0/3.0d0*flux_node(k,i+1)
              fright(k,i) =  1.0d0/3.0d0*flux_node(k,i  )+5.0d0/6.0d0*flux_node(k,i+1)-1.0d0/6.0d0*flux_node(k,i+2)
   ...
   !  		d_xxl(i) = (-1.0d0/8.0d0)*(d_xx(i-1)) + (6.0d0/8.0d00)*(d_xx(i)) + (3.0d0/8.0d0)*(d_xx(i+1))
		 ...
		 !  	d_yxl(i) = (-1.0d0/8.0d0)*(d_yx(i-1)) + (6.0d0/8.0d00)*(d_yx(i)) + (3.0d0/8.0d0)*(d_yx(i+1))
			...
			d_xxl(i) = (-1.0d0/6.0d0)*(d_xx(i-1)) + (5.0d0/6.0d00)*(d_xx(i)) + (1.0d0/3.0d0)*(d_xx(i+1))
			
			d_xxr(i) = (1.0d0/3.0d0)*(d_xx(i)) + (5.0d0/6.0d00)*(d_xx(i+1)) - (1.0d0/6.0d0)*(d_xx(i+2))

		  	d_xyl(i) = (-1.0d0/6.0d0)*(d_xy(i-1)) + (5.0d0/6.0d00)*(d_xy(i)) + (1.0d0/3.0d0)*(d_xy(i+1))
			
			d_xyr(i) = (1.0d0/3.0d0)*(d_xy(i)) + (5.0d0/6.0d00)*(d_xy(i+1)) - (1.0d0/6.0d0)*(d_xy(i+2))

		  	d_yyl(i) = (-1.0d0/6.0d0)*(d_yy(i-1)) + (5.0d0/6.0d00)*(d_yy(i)) + (1.0d0/3.0d0)*(d_yy(i+1))
			
			d_yyr(i) = (1.0d0/3.0d0)*(d_yy(i)) + (5.0d0/6.0d00)*(d_yy(i+1)) - (1.0d0/6.0d0)*(d_yy(i+2))

		  	d_yxl(i) = (-1.0d0/6.0d0)*(d_yx(i-1)) + (5.0d0/6.0d00)*(d_yx(i)) + (1.0d0/3.0d0)*(d_yx(i+1))
			
			d_yxr(i) = (1.0d0/3.0d0)*(d_yx(i)) + (5.0d0/6.0d00)*(d_yx(i+1)) - (1.0d0/6.0d0)*(d_yx(i+2))
	...
	implicit none
	double precision, parameter 		:: pi=acos(-1.0d0), beta =  0.0d0, D_parallel = 1.0d9
	integer								:: i, j, k
	...
	do j =1, NX
		...
		do i = -ghostp, NY+ghostp
			...
			cons_local(3,i) = cons(3,j,i)
					...
					d_xx(i) = D_parallel*(1.0d0+cons(1,j,i)**2.0d0)
					d_xy(i) = 0.0d0
					d_yx(i) = 0.0d0
					d_yy(i) = 1.0d0+cons(1,j,i)**2.0d0
...
double precision, parameter :: pi = acos(-1.0d0)
my_function = pi**2*(3*D_para*sin(3.14159265358979d0*x)**2*sin( &
      3.14159265358979d0*y)**2 - 2*D_para*sin(3.14159265358979d0*y)**2 &
      + D_para + 3*sin(3.14159265358979d0*x)**2*sin(3.14159265358979d0* &
      y)**2 - 2*sin(3.14159265358979d0*x)**2 + 1)*sin( &
      3.14159265358979d0*x)*sin(3.14159265358979d0*y)
	...
	open(unit=1,file='solution_4a_flux.plt',form='formatted',status='replace')
