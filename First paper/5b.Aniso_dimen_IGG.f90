	...
	double precision 					:: dx, dy, xmin, xmax, ymin, ymax, L
		...
		do i = 1, NY
			...
			do j = 1, NX
				...
				cons_exact(1,i,j) = (sin(x(i)*pi)*sin(y(j)*pi*4000.0d0))
			enddo
		enddo
	...
	dt = 1.0d010
		...
		do i = 1, NX
			...
			do j = 1, NY
				...
				d_xy =   0.0d0
				d_yy =   1.0d0
				Lr = (0.0005/sqrt(1.0d0+0.0005d0**2))/(2.0d0*acos(-1.0d0))
				Tr = Lr**2.0d0
...
! Upwind interpolation
  ...
  d_yy =   1.0d0
 	...
 	subroutine sourceterm(NX,NY,x,y,cons,flux_x,flux_y,residual,n_eqn,ghostp)
		...
		allocate (sourcet(3,1:NX,1:NY))
			do j = 1, NY
				...
				sourcet(1,i,j)		= sin(x(i)*pi)*sin(y(j)*pi*4000.0d0)*(pi**2)*(1.0d0+4000.0d0**2)
	...
	open(unit=1,file='Result-6a.plt')
