...
program hyperpoi3
		...
		integer 							:: i, j, z
		integer, parameter 					:: NX =384, n_eqn = 2, ghostp = 5						! No.of cells
		double precision, parameter			:: l1_target = 1.0d-12
		
		double precision 					:: dx, X(1:NX),xmin, xmax
		double precision 					:: cons(n_eqn,-3:NX+4),cons_old(n_eqn,-3:NX+4)
	    ...
	    call timestep		  (dx,dt,CFL)				! Steady state
...
! Runge Kutta step-3
	...
	double precision, parameter 		:: pi=acos(-1.0d0)
		  ...
		  nu 	= 	1.0d0
  		  ...
  		  Tr 	= 	Lr**2
  		  CFL 	=	0.99d0
  		  dt 	= 	CFL*dx/sqrt(nu/Tr)	! Currently it is unnecessary for required for future codes

  		  CFL 	=	0.72d0
  		  dt 	= 	CFL*dx				! It is much faster like this and i need to investigate it further. Optimum CFL?
	...
	subroutine initial(NX,cons,n_eqn)
		...
		integer 							:: NX, n_eqn
		double precision 					:: cons(n_eqn,-3:NX+4)
...
! Set right boundary condition
	...
	double precision 					:: cons(n_eqn,-3:NX+4),residual(n_eqn,1:NX)

	double precision   					:: flux_x_half(n_eqn,-3:NX+4),flux_node_x(n_eqn,-3:NX+4)
...
! 6th order difference formula. Midpoint to node 
	...
	subroutine reconstruction3 (NX,cons,flux, n_eqn,ghostp)
		...
		double precision 					:: cons(n_eqn,-3:NX+4)
		double precision 					:: flux(n_eqn,-3:NX+4)
		double precision 					:: consl(n_eqn,-3:NX+4), consr(n_eqn,-3:NX+4)

		double precision   					:: fleft(n_eqn,-3:NX+4), fright(n_eqn,-3:NX+4)

		double precision					:: righteigen(2,2), lefteigen(2,2),dissipation(n_eqn,-2:NX+2)
		double precision 					:: lambda1(-3:NX+4), lambda2(-3:NX+4)
		double precision  					:: delphi(-3:NX+4), delu(-3:NX+4)
		double precision 					:: alpha1, alpha2
		double precision 					:: SLm(-3:NX+4), SRp(-3:NX+4),laxmax
   ...
   !      	SLm(i) 		   = -1.0d0
	...
	double precision					:: A, L, M
			...
			source(2,i)		= -cons(2,i)
			residual(:,i)	= residual(:,i) + source(:,i)
	...
	double precision 		:: X(1:NX),cons(n_eqn,-3:NX+4)
	double precision		:: cons_exact(n_eqn,1:NX)
	...
	open(unit=9,file='Result-1a.plt',form='formatted',status='replace')
