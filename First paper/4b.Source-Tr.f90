...
!**********************************************************************
!    This program solves 2D Poisson equation by Hyperbolic approach
!	
...
!**********************************************************************
	...
	program hyperpoi2d
		...
		integer, parameter					:: NX = 16*16,NY = 16*16, n_eqn = 3, ghostp = 5 	
	...
	!***********************************************************************
			...
			call boundarycondition(NX,NY,cons,n_eqn,ghostp)
...
! Exact solution -1 
		 		! cons_exact(1,i,j) = dsin(x(i)*pi/L)*dcos(y(j)*pi/L)				
! Exact solution - 2
				cons_exact(1,i,j) = dsin(4.0d0*pi*x(i))*dsin(4.0d0*pi*y(j))
	...
	subroutine boundarycondition(NX,NY,cons,n_eqn,ghostp)
...
! Gradient variables for Neumann boundary conditions. Conservative variables 2 and 3 for boundaries
    ...
    do i = 1,NX  
    	do j = -1,-ghostp,-1
        	...
        	cons(:,i,j)   = 5.0d0 * cons(:,i,j+1) - 10.0d0*cons(:,i,j+2) + 10.0d0*cons(:,i,j+3) -5.0d0*cons(:,i,j+4) + cons(:,i,j+5)
	...
	do i = 1, NX
        ...
        cons(1,i,j+1) = (128.0d0/35.0d0)*(phi_top +(5.0d0/128.0d0)*cons(1,i,j-3) - (7.0d0/32.0d0)*cons(1,i,j-2) +&
    ...
    do i = 1, NX
        ...
        cons(:,i,j+2)  = cons(:,i,j-3) - 5.0d0*cons(:,i,j-2) + 10.0d0*cons(:,i,j-1) -10.0d0*cons(:,i,j)  +5.0d0*cons(:,i,j+1)
    ...
    do j = 1,NY
    	...
    	do i = -1,-ghostp,-1
        	...
        	cons(:,i,j)   = 5.0d0 * cons(:,i+1,j) - 10.0d0*cons(:,i+2,j) + 10.0d0*cons(:,i+3,j) -5.0d0*cons(:,i+4,j) + cons(:,i+5,j)
	...
	do j = 1, NY
        ...
        cons(1,i+1,j) = (128.0d0/35.0d0)*(phi_right +(5.0d0/128.0d0)*cons(1,i-3,j) - (7.0d0/32.0d0)*cons(1,i-2,j) +&
    ...
    do j = 1, NY
        ...
        cons(:,i+2,j)  = cons(:,i-3,j) - 5.0d0*cons(:,i-2,j) + 10.0d0*cons(:,i-1,j) -10.0d0*cons(:,i,j)  +5.0d0*cons(:,i+1,j)
	...
	double precision					:: a(0:NY), b(0:NY), c(0:NY), r(0:NY),temp(0:NY)
   ...
   !      	SLm(i) 		   = -1.0d0
 	...
 	subroutine sourceterm(NX,NY,x,y,cons,flux_x,flux_y,residual,n_eqn,ghostp)
		...
		Tr = Lr**2.0d0
			...
			do j = 1, NY
				...
				sourcet(1,i,j)		= 32.0d0*(pi)**2.0d0*dsin(4.0d0*pi*x(i))*dsin(4.0d0*pi*y(j))
	...
	open(unit=1,file='Result-4b.plt')
