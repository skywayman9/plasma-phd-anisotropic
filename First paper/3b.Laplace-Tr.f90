...
!**********************************************************************
	...
	program hyperpoi2d
		...
		integer, parameter					:: NTMAX=100000,l=2
		integer 							:: i, j, z, k, NT
		
		integer, parameter					:: NX = 16*l,NY = 16*l, n_eqn = 3, ghostp = 6 	
		...
		dy = (ymax - ymin)/(NY)
			...
			do i=-ghostp,NX+ghostp
	        	X(i)=(i-0.50d0)*dx+xmin
	    	enddo

	    	do j=-ghostp,NY+ghostp
	        	y(j)=(j-0.50d0)*dy+ymin
	    	enddo
...
! Exact solution - 2
				...
				cons_exact(1,i,j) = sinh(1.5*pi*y(j)/ L) / (sinh(1.5*pi))*sin(1.5*pi*x(i)/L)
...
! Gradient variables for Neumann boundary conditions. Conservative variables 2 and 3 for boundaries
	...
	do j = 1, NY
        ...
        cons(2,i+1,j) = (128.0d0/35.0d0)*(px_right +(5.0d0/128.0d0)*cons(2,i-3,j) - (7.0d0/32.0d0)*cons(2,i-2,j) +&
					   (35.0d0/64.0d0)*cons(2,i-1,j) -(35.0d0/32.d0)*cons(2,i,j))

		cons(1,i+1,j)  = cons(1,i-4,j) - 5.0d0*cons(1,i-3,j) + 10.0d0*cons(1,i-2,j) -10.0d0*cons(1,i-1,j)+5.0d0*cons(1,i,j)
	...
	do i = 1, NX
        ...
        cons(1,i,j+1) = (128.0d0/35.0d0)*(dsin(1.5d0*pi*(x(i)+x(i+0))*0.5d0 ) +(5.0d0/128.0d0)*cons(1,i,j-3) - (7.0d0/32.0d0)*cons(1,i,j-2) +&
 ...
 !    enddo
	...
	! 	cons(2,i+1,j) = (8.0d0*px_right +1.0d0*cons(2,i-1,j) -6.0d0* cons(2,i,j)) * (1.0d0/3.0d0)
 ...
 !    enddo
	...
	! 	cons(1,i,j+1) = (8.0d0*sin(1.5d0*pi*x(i)) +1.0d0*cons(1,i,j-1) -6.0d0* cons(1,i,j)) * (1.0d0/3.0d0)
 ...
 !    enddo
   ...
   !      	SLm(i) 		   = -1.0d0
	...
	open(unit=1,file='Result-3b.plt')
