...
!**********************************************************************
	...
	program hyperpoi2d
		...
		integer, parameter					:: NX = 20*l,NY = 20*l, n_eqn = 3, ghostp = 5 	
		...
		double precision 					:: cons_1(n_eqn,-ghostp:NX+ghostp,-ghostp:NY+ghostp),cons_2(n_eqn,-ghostp:NX+ghostp,-ghostp:NY+ghostp)

		double precision 					:: cons_3(n_eqn,-ghostp:NX+ghostp,-ghostp:NY+ghostp)
			...
			do k = 1,n_eqn
				...
				do j = 1, NY
					...
					do i = 1, NX
						...
						cons_1(k,i,j) = cons_old(k,i,j) + dt* residual(k,i,j)*0.5d0
	...
	!***********************************************************************	
			...
			call boundarycondition(NX,NY,cons_1,n_eqn,ghostp,x,y)
			
			call FX(NX,NY,dx,x,y,cons_1,flux_x,n_eqn,ghostp)
			
			call GY(NX,NY,dy,x,y,cons_1,flux_y,n_eqn,ghostp)

			call sourceterm(NX,NY,x,y,cons_1,flux_x,flux_y,residual,n_eqn,ghostp)
			...
			do k = 1,n_eqn
				...
				do j = 1, NY
					...
					do i = 1, NX
						...
						cons_2(k,i,j) = cons_old(k,i,j) + dt* residual(k,i,j)*0.5d0
	...
	!***********************************************************************
			...
			call boundarycondition(NX,NY,cons_2,n_eqn,ghostp,x,y)
			
			call FX(NX,NY,dx,x,y,cons_2,flux_x,n_eqn,ghostp)
			
			call GY(NX,NY,dy,x,y,cons_2,flux_y,n_eqn,ghostp)

			call sourceterm(NX,NY,x,y,cons_2,flux_x,flux_y,residual,n_eqn,ghostp)
			...
			do k = 1,n_eqn
				...
				do j = 1, NY
					...
					do i = 1, NX
						...
						cons_3(k,i,j) = cons_old(k,i,j) + dt* residual(k,i,j)*0.5d0
	...
	!***********************************************************************
			...
			call boundarycondition(NX,NY,cons_3,n_eqn,ghostp,x,y)
			
			call FX(NX,NY,dx,x,y,cons_3,flux_x,n_eqn,ghostp)
			
			call GY(NX,NY,dy,x,y,cons_3,flux_y,n_eqn,ghostp)

			call sourceterm(NX,NY,x,y,cons_3,flux_x,flux_y,residual,n_eqn,ghostp)
			...
			do k = 1,n_eqn
				...
				do j = 1, NY
					...
					do i = 1, NX
						...
						cons(k,i,j) = (1.0d0/3.0d0)*(-cons_old(k,i,j)+cons_1(k,i,j)+2.0d0*cons_2(k,i,j)+cons_3(k,i,j)) + (1.0d0/6.0d0)*(dt* residual(k,i,j) )
	...
	double precision 					:: dx, dy, xmin, xmax, ymin, ymax, L
		...
		do j = 1, NY
			...
			do i = 1, NX
				...
				cons_exact(1,i,j) = sin(pi*y(j)*0.5d0)*sin(pi*x(i)*0.5d0)
	  ...
	  Lr = 1.0d0/(2.0d0*acos(-1.0d0))
		...
		do i = 1, NX
			...
			do j = 1, NY
				...
				x_velocity =  ABS(sqrt(1.0d0/Tr))
				y_velocity =  ABS(sqrt(1.0d0/Tr))
...
! Fifth order boundary conditions
	...
	do j = 1, NY
        ...
        cons(1,i+1,j) = (128.0d0/35.0d0)*(sin(pi*y(j)*0.5d0)*sin(pi*1.0d0*0.5d0)+(5.0d0/128.0d0)*cons(1,i-3,j) - (7.0d0/32.0d0)*cons(1,i-2,j) +&
	...
	do i = 1, NX
        ...
        cons(1,i,j+1) = (128.0d0/35.0d0)*(sin(pi*1.0d0*0.5d0)*sin(pi*x(i)*0.5d0) +(5.0d0/128.0d0)*cons(1,i,j-3) - (7.0d0/32.0d0)*cons(1,i,j-2) +&
	...
	double precision					:: alpha_l, alpha_r, theta_l, theta_r, Lr, Tr
	...
	double precision					:: rl, rr, phi_l, phi_r

    double precision					:: phi_l_inv, phi_r_inv, Lr, Tr

    double precision					:: alpha_l, alpha_r, theta_l, theta_r
   ...
   !      	SLm(i) 		   = -1.0d0
	...
	!***********************************************************************
 	...
 	subroutine sourceterm(NX,NY,x,y,cons,flux_x,flux_y,residual,n_eqn,ghostp)
		...
		double precision 					:: residual(n_eqn,-ghostp:NX+ghostp,-ghostp:NY+ghostp), Tr, Lr
...
! 		enddo
			...
			do j = 1, NY
				...
				sourcet(1,i,j)		=  pi**2.0d0*0.5d0*sin(pi*y(j)*0.5d0)*sin(pi*x(i)*0.5d0)
				sourcet(2,i,j)		= -cons(2,i,j)
				sourcet(3,i,j)		= -cons(3,i,j)
	...
	open(unit=9,file='5CR_center.txt',form='formatted',status='replace')
	
	! Write the diagonal data to plot with the exact solution :) or the center line
	...
	do j = 1, NY
	    ...
	    ! do i=1,NX
	    	...
	    	! if(i.eq.50) then
    		i = NX/2
        	write(9,'(5es25.10)') x(i),y(j), cons(1,i,j),cons_exact(1,i,j)
    ...
    enddo
 	...
 	! Only Potential is being plotted . I am not checking the gradient variables
 	
 	open(unit=21, file="exact.txt",action="write",status="replace")

	open(unit=22, file="5CR_Potential.txt",action="write",status="replace")
	open(unit=23, file="5CR_ux.txt",action="write",status="replace")
	open(unit=24, file="5CR_uy.txt",action="write",status="replace")


		do i=1,NX
		    
		    write(21,'(1600F14.7)')(cons_exact(1,i,j),j=1,NY)
 	...
 	close(21)
		...
		do i=1,NX
		    ...
		    write(22,'(1600F14.7)')(cons(1,i,j),j=1,NY)
		    write(23,'(1600F14.7)')(cons(2,i,j),j=1,NY)
		    write(24,'(1600F14.7)')(cons(3,i,j),j=1,NY)
